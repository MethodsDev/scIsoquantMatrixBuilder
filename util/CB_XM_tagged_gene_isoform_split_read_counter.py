...
def main():
    ...
    usage = "usage: {} read_assignments.tsv.CB_UMI_info.sorted.w_reads_split.cell_gene_iso_sorted.tsv output_prefix".format(sys.argv[0])
    ...
    ofhs['ofh_cell_isoform_counts'] = open(output_prefix + ".cell_isoform_counts.tsv", "wt")
    ofhs['ofh_cell_unambig_read_isoform_counts'] = open(output_prefix + ".cell_unambig_isoform_counts.tsv", "wt")
...
def process_cell_gene_reads(reads_list, ofhs):
    ...
    for read in reads_list:
        read_id, cell_barcode, UMI, gene_id, transcript_id, gene_symbol, frac_read_assigned = read
        frac_read_assigned = float(frac_read_assigned)
        
        assert gene_id == gene
        if gene_id != gene_symbol:
            assert gene_symbol == gene_sym, "Error conflicting gene symbol info for gene_id {} with {} vs {}\nfor reads:{}".format(
    ...
    # add to outputs
    gene_tok = gene_sym + "^" + gene
    print("\t".join([cell, gene_tok, f"{gene_sum_reads:.2f}"]), file=ofhs['ofh_cell_gene_counts'])

    for isoform, count in iso_total_read_count.items():
        isoform_tok = gene_sym + "^" + isoform
        print("\t".join([cell, isoform_tok, f"{count:.2f}"]), file=ofhs['ofh_cell_isoform_counts'])

    for isoform, count in iso_total_unambig_count.items():
        isoform_tok = gene_sym + "^" + isoform
        print("\t".join([cell, isoform_tok, f"{count:.2f}"]), file=ofhs['ofh_cell_unambig_read_isoform_counts'])
