...
def main():
    ...
    parser = argparse.ArgumentParser(description="generate sparse matrices from sc-IsoQuant outputs",
                                     formatter_class=argparse.ArgumentDefaultsHelpFormatter)

    parser.add_argument("--sample_id", type=str, required=True, help="sample_id")
    parser.add_argument("--bam", type=str, required=True, help="input bam file - needs CB and XM tags")
    parser.add_argument("--transcript_model_reads", type=str, required=True, help="isoquant transcript_model_reads.tsv.gz file")
    parser.add_argument("--transcript_models_gtf", type=str, required=True, help="Isoquant transcript_models.gtf.gz file")
    ...
    transcript_models_gtf = args.transcript_models_gtf
    ...
    # extract CB and UMI from bam
    read_CB_UMIs_filename = os.path.join(intermediates_dir, "read_CB_umi.tsv")
    cmdstr = " ".join([ os.path.join(utildir, "bam_read_name_CB_UMI_extractor.py"),
                     bam_filename,
                     f"| sort -u > {read_CB_UMIs_filename}"])
    cmdstr = f'/bin/bash -c "set -eof pipefail; {cmdstr}"'
    
    pipeliner.add_commands([Command(cmdstr, "read_CB_UMIs.ok")])


    # get gene/trans mappings and gene name info:
    gene_trans_ids_filename = os.path.join(intermediates_dir, "gene_trans_info.tsv")
    cmdstr = " ".join([os.path.join(utildir, "gtf_to_gene_trans_info.py"),
    ...
    read_CB_UMI_gene_trans_info_filename = os.path.join(intermediates_dir, "read_CB_UMI_gene_trans_info.tsv")
    cmdstr = " ".join([os.path.join(utildir, "merge_gene_trans_CB_UMI_counts.Rscript"),
                       f"--transcript_model_reads {transcript_model_reads_filename}",
                       f"--read_CB_UMIs {read_CB_UMIs_filename}",
    ...
    read_info_sorted_by_readname_gene_filename = os.path.join(intermediates_dir, "read_info.sorted_rname_gene.tsv")
    ...
    reads_split_filename = os.path.join(intermediates_dir, "read_info.w_reads_split.tsv")
    cmdstr = " ".join([os.path.join(utildir, "CB_XM_tagged_gene_isoform_read_splitter.py"),
    ...
    cmdstr = " ".join(["sort -k2,2 -k4,4 -k5,5 ",
                       reads_split_filename,
                       ">",
    ...
    cmdstr = " ".join([ os.path.join(utildir, "CB_XM_tagged_gene_isoform_split_read_counter.py"),
                        sorted_split_reads_filename,
                        output_prefix])
    ...
    cmdstr = " ".join([ os.path.join(utildir, "CB_XM_tagged_IsoQuant_read_assignments_to_sparse_matrix.R"),
